...
static void setway(uint32_t level, uint32_t clean_it)
{
    ...
    COP_WRITE2(COP_SYSTEM, "2", COP_CACHE_CONTROL_ARMv7, csselr);
    
    // Get size details from current cache size id register
    COP_READ2(COP_SYSTEM, "1", COP_CACHE_CONTROL_ARMv7, ccsidr);
    ...
    for (way = num_ways - 1; way >= 0 ; way--) {
        for (set = num_sets - 1; set >= 0; set--) {
            ...
            if(clean_it)
                COP_WRITE(COP_SYSTEM, COP_DCACHE_CLEAN_INVALIDATE_LINE, setway);
            else
                COP_WRITE(COP_SYSTEM, COP_DCACHE_INVALIDATE_LINE, setway);
     ...
     */
    COP_WRITE(COP_SYSTEM, COP_DATA_SYNC_BARRIER,0);
}
...
static void mem_icache_invalidate(BOOL clean_it)
{
    ...
    COP_READ2(COP_SYSTEM, "1", COP_CACHE_CONTROL_ARMv7_CLIDR, clidr);
