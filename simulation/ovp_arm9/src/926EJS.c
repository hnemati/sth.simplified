                    ...
                    icmProcessorP *cpu_, icmBusP *bus_) 
{
    ...
    icmAddStringAttr(processorAttrList, "variant", "ARM926EJ-S");
...
//        icmDebugThisProcessor(processor);
    ...
    icmMemoryP ram = icmNewMemory("ram0", 0x7, 1024 * 1024 * 16-1);        
   ...
   /* if(strcmp(ICM_VERSION, "1.4.1") >= 0 ) {
    	icmAddStringAttr(usartAttrList, "log", "1");
    }
    else {
    	icmAddDoubleAttr(usartAttrList, "log", 1);
    }*/
    icmAddStringAttr(usartAttrList, "console", "1");     // or log
    ...
    *cpu_ = processor;
    *bus_ = bus;
    printf("\nOVP IntegratorCP platform created\n");
}
